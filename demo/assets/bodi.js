...
SOURCE CODE CAN BE FOUND - https://github.com/MarketingPipeline/Bodi/
*/ 
// Clock 
...
SubtitleBtn.onclick = (event) => {ShowSubtitlesSettingsMenu};



pauseBtn.onclick = (event) => {togglePause};



playBtn.onclick = (event) => {togglePlay};
NextBtn.onclick = (event) => {NextItem};  

MovieInfoBtn.onclick = (event) => { MovieInfo};


Video_Settings_Btn.onclick = (event) => {VideoSettings_Menu};
Video_Settings_Menu_Items.onclick = (event) => { VideoSettings_Menu_Click};



video.addEventListener('timeupdate', updateProgress);

progressSlider.onclick = (event) => { setProgress};  
...
function KeyBoardCancel(){
 ...
 AlbumCoverContainer.style.display = "block";      AlbumCoverContainer.style.backgroundImage="red";
...
var SpaceBar = document.getElementById('test');
let checkCaps = false;
...
const caps = function () {
  if (checkCaps == false) {
    for (let i = 0; i < alphabet.length; i++) {
      ...
      SpaceBar.style.background = "#1085a8";
      SpaceBar.style.borderRadius = "100%";
      SpaceBar.style["boxShadow"] = "0 0 5px #1085a8";
    }
    checkCaps = true;
  } else {
    for (let i = 0; i < alphabet.length; i++) {
      ...
      SpaceBar.style.borderRadius = "0%";
      SpaceBar.style["boxShadow"] = "0 0 0px";
      SpaceBar.style.background = "#192123";
    }
    checkCaps = false;
  }
};
